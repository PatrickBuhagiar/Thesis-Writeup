...
\title{Financial Forecasting}
\date{Version 2.18, 2016-December-05}
...
\parencite{Lamport1994}.  It is very different from \textsc{wysiwyg},
which has been characterised as ``What you see is all you've
got''\footnote{\textcite[p7, Footnote~1]{Lamport1994} says that ``Brian
  Reid attributed this phrase to himself and/or Brian Kernighan''.}

\LaTeXe\ is built on top of Donald Knuth's \TeX\ \parencite{Knuth1984}.
...
\parencite{Knuth1997,Knuth1998a,Knuth1998b}; take a look at them to see
...
\begin{description}
\item[\textcite{Lamport1994}] The original source.  It has a
  reasonable reference manual, but can be terse.  It does not cover
  package and class writing, nor does it cover more than a handful of
  useful packages.  It describes the deprecated \BibTeX{}, but the
  information on bibliographic databases is still relevant; it also
  describes index making programs.
\item[\textcite{KopkaDaly1999}] A comprehensive reference; it covers
  everything except the many add-on packages.  Most people use this as
  their primary reference.
\item[\textcite{MittelbachGoossensBraamsCarlisleRowley2004}] A guide to
  many of the most useful add-on packages and classes.
\item[\textcite{GoosensRahtzMittelbach1997}] A slightly dated guide to
  packages for graphics.
\item[\textcite{GoosensRahtzGurariMooreSutor1999}] A slightly dated guide
...
\item[The Not So Short Introduction to \LaTeXe\ 
  \parencite{OetikerPartlHynaSchlegl2002}]\
...
\item[Math mode \parencite{Voss2007}] A detailed explanation of
  typesetting mathematics in \LaTeXe.
\item[The Comprehensive \LaTeX\ Symbol List \parencite{Pakin2005}] An
...
\item[Packages in the `graphics' bundle \parencite{Carlisle1999}] A bit
  out of date (it does not describe PDF extensions), but a useful
  introduction.
\item[Hypertext marks in \LaTeX\ \parencite{RahtzOberdiek2003}] Turn
  on hypertext features, via the \lstinline|hyperref| package.
...
\item[The \KOMAScript\ bundle \parencite{KohmMorawski2003}]\
...
\newcommand{\wwwr}[2]{\item[\href{#1}{#2} {\footnotesize\textless\url{#1}\textgreater}]}
\begin{description}
...
\wwwr{http://texdoc.net/}{TeXdoc Online}
...
\wwwr{http://en.wikibooks.org/wiki/LaTeX}{A wiki for \LaTeXe{}}
  ...
  As good or as bad as a Wiki can be.
...
\item Create a \LaTeXe{} source file, and any others needed, such as a
  Biber database file, figures, and so on.
...
\item Run \lstinline{biber}.  (This creates a file containing the
  references.)
...
necessary; rerunning \lstinline{biber} is only necessary if new
citations are inserted or if an entry in the bibliographic database
changes.
...
There are many packages available; see examples given in
...
\begin{lstlisting}[
  ...
  \newenvironment*{mq}[1]
    ...
    {\par\hspace*{\fill}---\parencite{\cl}\end{quote}}% end code
\end{lstlisting}
...
protocol is due to \textcite{OtwayRees1987}; the notation $\enc{K}{M}$,
...
\item[Back matter] The references and appendices.
  ...
  references.  The best way is through Bib\LaTeX, a format for
...
Biber databases that cause problems when preparing the bibliographic
...
\item[Parenthesized] 
  \begin{tabular}[t]{l@{~\textrm{generates}~}l}
    \lstinline|\parencite{Joyce:FW}|&\parencite{Joyce:FW}
    \\
    \lstinline|\parencite[\S4]{Joyce:FW}|&\parencite[\S4]{Joyce:FW}
    \\
    \lstinline|\parencite[see][\S4]{Joyce:FW}|&\parencite[see][\S4]{Joyce:FW}
  \end{tabular}
\item[Textual]
  \begin{tabular}[t]{l@{~\textrm{generates}~}l}
    \lstinline|\textcite{Joyce:FW}|&\textcite{Joyce:FW}
    \\
    \lstinline|\textcite[\S4]{Joyce:FW}|&\textcite[\S4]{Joyce:FW}
    \\
    \lstinline|\textcite[see][\S4]{Joyce:FW}|&\textcite[see][\S4]{Joyce:FW}
  \end{tabular}
\end{description}
...
example:
`\ldots\lstinline|important modernist works~\parencite{Elliot:WL,Joyce:FW}|'
...
Each float can have a symbolic label for use by the cross-referencing
mechanism of \LaTeXe.
...
the PSNFSS bundle \parencite{Schmidt2004} (for roman shape: Hermann Zapf's
...
The citation mechanism in \lstinline{UoYCSproject} is utilises the
modern Bib\LaTeX{}.  The command \lstinline{\parencite} is used for
parenthesised citations and \lstinline{\textcite} for citations as
text.  See \autoref{sec:bibliographies}.

The command \lstinline{\cite} should not be used.
