...
\section{Tools}
As discussed in Section \ref{sec:discussion}, the most suitable approach to confirm the hypotheses mentioned in Section \ref{sec:objectives} was to utilise machine learning techniques, particularly feed forward networks and ensemble learning. Naturally, a decision had to made on the programming language and library to be used. With regards to programming languages, Java, R, Matlab and Python were taken into consideration. As for machine learning tools, Weka, TensorFlow, Theano, DL4J (Deep learning for Java) and other packages and functionality found within Matlab and R were taken into consideration. Ultimately, it was decided to use TensorFlow with Python, primarily due to having a certain level of familiarity with Python as a language, and the popularity of TensorFlow in the industry and research community. 

TensorFlow is an open source software library for high performance numerical computation, but is widely used for machine learning applications, such as neural networks. It was developed by the Google Brain team, thus it is heavily used by Google itself in its own applications. Despite only having been released in 2015, it has already established itself as one of the most popular libraries in the industry. The excellent documentation, the ease of deploying across different platforms such as CPUs, GPUs and TPUs, the endless use cases that TensorFlow can be applied to, the good readability and high level of abstraction are some of the reasons why TensorFlow rose to fame so quickly. Compared to other libraries, such as Theano, it requires less functionality hacking to tailor to specific problems, which means that developers can apply and try different architectures and parameters quickly and effectively. Consequently, TensorFlow has developed a vibrant community around it, which means that it will continue to be maintained and improved for years to come. These reasons ultimately made TensorFlow the library of choice for this work.         

TensorFlow was specifically built to run with Python, but also offers APIs for Java, C and Go. These however are not as extensive as the Python APIs. Despite having experience in all these languages, Python was made the programming language of choice. Python is a dynamically typed language that supports several programming paradigms such as object-oriented, imperative and functional paradigms. It also serves as a great scripting language, making it suitable for quick and easy experimentation. It is free and open source, unlike Matlab, and is available on several operating systems. The version of Python used in this work was version 3.6.5, while version 1.8.0 was used for TensorFlow.
